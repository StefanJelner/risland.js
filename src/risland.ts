...
export default class RIsland<IState extends Record<string, any>> {
            ...
            class extends HTMLElement {
                ...
                constructor() {
                    ...
                    const shadow = this.attachShadow({
                        // since verion 0.14 the shadow DOM is closed, because it does not make sense to allow DOM
                        // manipulations inside the island, because those changes would be lost in the next rendering
                        // cycle anyway. besides it is considered an antipattern.
                        // Jest is having trouble with this and it would be necessary to write some spies or do
                        // complex monkey patching, so in the Jest context the shadow DOM is open. The solution is
                        // not perfect, because one could just mock process.env.JEST_WORKER_ID, but it should work
                        // in most cases.
                        mode:
                            // optional chaining does not work with "process", this is why it has to be done in the
                            // 1985 old school way, like on the C64.
    ...
    private _initialConfig: IRIslandConfig<IState> = {
        ...
        partials: {},
        shouldUpdate: (state: Readonly<IState>, nextState: Readonly<IState>, deepEqual: typeof equal) =>
    ...
    private _internalSqrlConfig: SqrlConfig = {
        ...Sqrl.defaultConfig,
        ...this._enforcedConfig.squirrelly,
    };
    ...
    private _delegationFuncs: Partial<
        Record<
            TRIslandDelegationFuncsKey,
            {
                capture: boolean;
    ...
    private _supportsNativeCloning = typeof window.structuredClone === 'function';
    ...
    constructor(config: Partial<IRIslandConfig<IState>>) {
        this._config = deepmerge.all<IRIslandConfig<IState>>(
            [
                ...
                // we directly use klona here, because structuredClone() is not capable of cloning DOM nodes or
                // functions.
                klona(config),
        ...
        Object.keys(this._config.partials).forEach((partial: string) => {
            Sqrl.templates.define(
                partial,
                Sqrl.compile(this._getTemplate(this._config.partials[partial]), {
        ...
        (Object.keys(this._config.delegations) as Array<TRIslandEventNames>).forEach(
            (
                ...
                commaSeparatedEventNames
                    .trim()
                    .split(/\s*,\s*/g)
                    .forEach((eventName: TRIslandEventNamesThrottled) => {
                                        ...
                                        (selector: string) => {
                                            ...
                                            if ($closest !== null) {
                                                this._config.delegations[commaSeparatedEventNames][selector](
                                                    event,
                                                    $closest,
                                                    this._clone<IState>(this._state),
                        ...
                        if (throttling.throttled === true) {
                            if ('ms' in throttling) {
                                ...
                                this._delegationFuncs[funcName].func = rafThrottle(
                                    this._delegationFuncs[funcName].func,
                                );
                            }
                        }
    ...
    public unload(): void {
        ...
        Object.keys(this._delegationFuncs).forEach((funcName: TRIslandDelegationFuncsKey) => {
            this._config.$element.removeEventListener(funcName.split(/:/g)[0], this._delegationFuncs[funcName].func, {
                capture: this._delegationFuncs[funcName].capture,
            });
        });
        ...
        this._config.unload(this._clone<IState>(this._state));
    }
        ...
        const tmpState =
            typeof nextState === 'function'
                ? // we have to work with a clone here, otherwise it would be possible to brutally mutate the state.
                  nextState(this._clone<IState>(this._state))
                : nextState;
        ...
        const tmpMergedState: IState =
            'customMerge' in this._config.deepmerge
                ? // if a custom merging algorithm is provided, it is possible to manipulate the state directly
                  ...
                  deepmerge(this._clone<IState>(this._state), tmpState as Partial<IState>, this._config.deepmerge)
                  ...
                  this._config.shouldUpdate(
                      ...
                      'customMerge' in this._config.deepmerge
                          ? // if a custom merging algorithm is provided, tmpMergedState already contains a clone of
                            ...
                            tmpMergedState
                          : this._clone<IState>(tmpMergedState),
                      equal,
                  );
    ...
    private _render(): void {
        ...
        if (this._errors.length > 0) {
            this._config.$element.innerHTML = this._compiledError({ errors: this._errors }, this._internalSqrlConfig);
            /* eslint-disable no-console */
            console.error(
                this._compiledConsole(
                    {
        ...
        morphdom(this._config.$element.firstChild, checkedTemplate, {
    ...
    private _loadOrUpdate(): void {
        const state = this._clone<IState>(this._state);
    ...
    /**
     * Clones a given object natively (with fallback)
     *
     * @param object object to clone
     * @returns cloned object
     */
    private _clone<T>(object: T): T {
        if (this._supportsNativeCloning === true) {
            // structuredClone() is not capable of cloning DOM nodes and functions. usually it is unlikely that
            // the state contains such data, but for the rare cases after structuredClone() throws an exception,
            // klona is tried as a fallback.
            ...
            // one might argue, that this approach slows down the process, but in most cases, the use of
            // structuredClone() should bring a significant speed advantage. the edge cases are not really antipatterns
            // but usually have a smell.
            let clone;
